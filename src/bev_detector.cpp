...
BevResult BevDetector::Update(const std::vector<Eigen::Vector3d> &points_map,
  ...
  // Two filters, single pass: (1) map-frame z band, then (2) track mask.
  //   1. z band: keep z_min <= z <= z_max. Below z_min = ground/floor,
  //      above z_max = ceiling/overhang. A fixed band replaces RANSAC ground
  //      fitting -- localization gives a reliable map-frame z, so the floor is
  //      at a known height and doesn't need per-frame plane estimation.
  //   2. track mask: erosion by distance to the nearest non-track (wall/
  //      off-track) cell. Off-track points are 0 (dropped); on-track points
  //      grow with distance from the wall, so only points >= track_dist_min
  //      from every wall survive. Points off the loaded grid get +inf from
  //      DistToBlack -- std::isfinite excludes them (inf <= track_dist_min is
  //      always false, so they'd otherwise slip through unfiltered).
  std::unordered_map<int64_t, Obs> cells;
  cells.reserve(points_map.size() / 8 + 16);
  out.obstacle_points.reserve(points_map.size() / 8 + 16);
  ...
  for (const auto &p : points_map) {
    ...
    if (p.z() < p_.z_min) {  // below the band = ground/floor
